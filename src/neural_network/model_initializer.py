...
from tensorflow.keras.layers import Dense, BatchNormalization, Dropout, SpatialDropout1D, Conv1D, MaxPool1D, Flatten
from tensorflow.keras.optimizers import SGD
from tensorflow.keras.initializers import lecun_normal, RandomNormal, Constant
from keras.layers.merge import concatenate
from neural_network.model_evaluation import combined_model_evaluation, model_kfold_evaluation
...
def _fnn_builder(config):
    ...
    model.compile(loss='categorical_crossentropy', optimizer=SGD(learning_rate=1e-3, decay=1e-6, momentum=0.9, nesterov=True), metrics=['accuracy'])
    plot_model(model, show_shapes=True, to_file='cosmos_fnn.png')
...
def _dcnn_builder(config):
    ...
    for i in range(config.layers_num):
        ...
        #x = SpatialDropout1D(config.dropout_cnn)(x)
    x_flatten = Flatten()(x)
    ...
    model.compile(loss='categorical_crossentropy', optimizer=SGD(learning_rate=1e-3, decay=1e-6, momentum=0.9, nesterov=True), metrics=['accuracy'])
    plot_model(model, show_shapes=True, to_file='cosmos_cnn.png')
...
def _test_build(local, lc_train_dataset, aux_train_dataset, lc_valid_dataset, aux_valid_dataset, lc_test_dataset, aux_test_dataset, config_fnn, config_cnn):
    ...
    #combined_model_evaluation(combined_model, lc_train_dataset, lc_valid_dataset, lc_test_dataset, aux_train_dataset, aux_valid_dataset, aux_test_dataset, config_cnn)
    return
