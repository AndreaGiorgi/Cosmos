...
def dataset_parser(tf_example, label_to_id, config, reverse_prob):
        ...
        for feature_name, value in parsed_features.items():
            if feature_name == config.label_feature:
                ...
                with tf.control_dependencies([assert_known_label]):
                    label_id = tf.identity(label_id) #? boh
...
def dataset_builder(tfrecords, config, labels, reverse_prob, shuffle_buffer):
   ...
   #     dataset = dataset.repeat(dataset)
    ...
    dataset = dataset.map(partial(dataset_parser, label_to_id, config = config, labels = labels, reverse_prob = reverse_prob), num_parallel_calls=AUTOTUNE)
