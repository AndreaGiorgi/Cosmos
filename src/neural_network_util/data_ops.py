...
def retrive_dataset(tfrecords, config, batch_size, labels, reverse_prob, shuffle_filenames, shuffle_buffer, repeat_dataset):
    ...
    def dataset_parser(tf_example):
        ...
        data_fields[config.label_feature] = tf.io.FixedLenFeature([], tf.string)
        
        #? Parse the features.
        parsed_features = tf.io.parse_single_example(tf_example, features=data_fields)
        ...
        for feature_name, value in parsed_features.items():
            if labels and feature_name == config.label_feature:
                ...
                assert_known_label = tf.debugging.Assert(
                    ...
                    ["Unknown Label:", value])
                with tf.control_dependencies([assert_knwon_label]):
    ...
    dataset = filname_dataset.flat_map(tf.Data.TFRecordsDataset)
    if shuffle_buffer > 0:
